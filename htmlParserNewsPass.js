                      ...
                      if(itr < 1000){ 
                        ...
                        if(textintd.length >= 8){
                          ...
                          if ( shareFunc.isNeedTopics(strTitle) && !isDWSETTSDmai )  {
                            arrNews.push(shareFunc.newsTodayObject(strTime,strSymbol,strSource,strTitle))
...
async function processNewsByPage(url,intPage){
    ...
    //     let arr = wrapHtmlParser(this.responseText,intPage)
    //     arrAllPassNews = [...arrAllPassNews , ...arr]
  
    //   }else if ( this.response && this.status == 0){
    //     console.log('Error : in processNewsByPage')
    //   }
    // }
...
async function getAllNumberOfPageAndProcess(){
    ...
    //     intPage = substringTogetPageNumber(this.responseText)
    //     console.log('Passnews : page' , intPage)
    //     processPassNews(intPage)
    //     //console.log(this.responseText.slice(0,400))
    ...
    // try {
    //   xhttp.open("GET", urlnews+paranews, true)
    //   xhttp.send()
    // }catch (err) {
    //   console.log(err.message)
    // }
    ...
    //console.log('getAllNumberOfPageAndProcess page:', intPage)
